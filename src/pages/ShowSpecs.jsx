...
export default function ShowSpecs() {
  ...
  const [perfOnly, setPerfOnly] = useState(false); // NEW: Performance-only toggle
  ...
  const filtered = useMemo(() => {
    ...
    return shows.filter((s) => {
      ...
      // NEW: Performance-only filter
      const matchPerformance = !perfOnly || showType.includes("performance");
      ...
      return (
        ...
        matchVenue &&
        matchType &&
        matchPerformance && // NEW condition
        matchMonth &&
        matchFuture &&
        matchUndated &&
        matchCancelled
  ...
  return (
    <div className="page">
      ...
      >
        ...
        {/* NEW: Performance-only filter button */}
        <button
          onClick={() => setPerfOnly((v) => !v)}
          style={{
            ...buttonStyle,
            background: perfOnly ? "#333" : "#1d1d1f",
            textAlign: "center",
            minWidth: "170px",
          }}
        >
          {perfOnly ? "✓ " : ""}Performance only
        </button>
      </div>
