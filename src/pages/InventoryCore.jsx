...
// unique key for a specific inventory row
function itemKey(it) {
  return [
    norm(it.category),
    norm(it.item || it.name),
    norm(it.type),
    norm(it.location),
  ].join("||");
}
...
export default function InventoryCore({
  ...
  const [batchItems, setBatchItems] = useState([]); // { key, item, maxAvailable, qty }
  ...
  function toggleBatchItem(it) {
    ...
    setBatchItems((prev) => {
      ...
      const maxAvailable = it._available ?? it.available ?? 0;
      if (maxAvailable <= 0) return prev; // nothing to add
      return [
        ...prev,
        {
          key,
  ...
  function isInBatch(it) {
    const key = itemKey(it);
    return batchItems.some((x) => x.key === key);
  }

  // ----------------------------------------------------
  // checkout helpers
  // ----------------------------------------------------
  async function doSingleCheckout(formData) {
  ...
  async function handleCheckoutSubmit(formData) {
    setSaving(true);
    try {
      const { inv, inuse } = await doSingleCheckout(formData);
      setInventory(inv);
      setInUse(inuse);
      setModalItem(null);
    } catch (e) {
  ...
  // batch checkout
  async function handleBatchCheckoutSubmit(payload) {
    // payload: { rows: [{ key, item, qty }], eventTitle, location, name }
    setSaving(true);
    try {
      let latestInv = inventory;
      let latestInUse = inUse;
      ...
      for (const row of payload.rows) {
        ...
        const it = row.item;
        const { inv, inuse } = await doSingleCheckout({
      ...
      setInUse(latestInUse);
      setBatchItems([]);
    } catch (e) {
      alert("Batch checkout failed: " + e.message);
    } finally {
      setSaving(false);
    }
  }
  ...
  // ----------------------------------------------------
  // return / checkin (partial qty supported via ReturnModal)
  // ----------------------------------------------------
      ...
      {batchItems.length > 0 && (
        ...
        >
          <div className="text-sm">
            <strong>{batchItems.length}</strong> item
            {batchItems.length === 1 ? "" : "s"} selected for batch checkout.
          </div>
                                  ...
                                  <button
                                    onClick={() =>
                                      setReturnTarget({
                                        ...
                                        maxQty: Number(r.qty || 0),
                                        itemName: r.item || r.name,
                                      })
                                    }
                                    className="btn ghost"
                                    style={{
                        ...
                        .map((it, idx) => (
                          <div key={idx} className="inv-variant-row">
                            ...
                            >
                              ...
                              <button
                                disabled={it._available === 0}
                                onClick={() => toggleBatchItem(it)}
                                className={isInBatch(it) ? "btn" : "btn ghost"}
                                style={{
                                  fontSize: "0.75rem",
                                  padding: "4px 10px",
                                  opacity: it._available === 0 ? 0.5 : 1,
                                }}
                              >
                                {isInBatch(it) ? "Selected" : "Add"}
                              </button>
                            </div>
                          </div>
                        ))}
      ...
      {returnTarget && (
        <ReturnModal
          ...
          onSubmit={(qty) => {
            ...
            handleReturn(returnTarget.checkoutId, qty);
          }}
        />
      )}

      {/* BATCH CHECKOUT MODAL */}
      {batchModalOpen && batchItems.length > 0 && (
        <BatchCheckoutModal
          items={batchItems}
          onClose={() => setBatchModalOpen(false)}
          onSubmit={(payload) => {
            setBatchModalOpen(false);
            handleBatchCheckoutSubmit(payload);
          }}
          saving={saving}
          shows={shows}
          currentUser={currentUser}
...
/* =========================================================
   Return modal (partial check-in)
   ========================================================= */
function ReturnModal({ target, onClose, onSubmit }) {
  const max = target.maxQty || 1;
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal-card" style={{ maxWidth: 360 }}>
        <div className="modal-header">
          <div className="modal-title">
            Return: {target.itemName} (max {max})
          </div>
          <button onClick={onClose} className="modal-close">
            ×
          </button>
...
/* =========================================================
   Batch checkout modal (multiple items)
   ========================================================= */
function BatchCheckoutModal({
  items,
  onClose,
  onSubmit,
  saving,
  ...
  const [rows, setRows] = useState(
    () =>
      items.map((x) => ({
        key: x.key,
        item: x.item,
        max: x.maxAvailable ?? x.item._available ?? 0,
        qty: x.qty ?? 1,
      })) || []
  );
  ...
  function submit() {
    ...
    if (!hasQty) return;
    onSubmit({ rows, eventTitle, location, name });
  }
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal-card" style={{ maxWidth: 520 }}>
        <div className="modal-header">
          <div className="modal-title">
            Batch checkout ({rows.length} item
            {rows.length === 1 ? "" : "s"})
          </div>
          <button onClick={onClose} className="modal-close">
            ×
        ...
        <div className="modal-body">
          <div className="muted" style={{ marginBottom: "0.75rem" }}>
            Set a quantity for each item (0 to skip), then fill in the show
            details below.
          </div>
            ...
            {rows.map((r) => (
              ...
              >
                <div style={{ fontWeight: 600 }}>
                  {r.item.item || r.item.name}
                  {r.item.type ? ` (${r.item.type})` : ""}
                </div>
                <div className="muted">
                  {r.item.location || "No location"} • Available: {r.max}
                </div>
                <div
                  style={{
                    display: "flex",
